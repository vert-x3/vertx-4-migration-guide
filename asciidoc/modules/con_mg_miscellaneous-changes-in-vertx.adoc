...
The following section describes miscellaneous changes in {VertX} {4}.
...
{VertX} {4} supports Java 11. This Java version does not support isolated class loading.
In {VertX} {4}, isolated class loading will be supported for Java 8. However, the results may not be as expected.

== Removed hook methods from context

The methods `Context.addCloseHook()` and `Context.removeCloseHook()` methods have been removed from the `Context` class.
