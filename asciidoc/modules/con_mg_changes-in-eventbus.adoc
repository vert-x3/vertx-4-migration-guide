...
The `EventBus.send(..., Handler<AsyncResult<Message<T>>>)` and `Message.reply(..., Handler<AsyncResult<Message<T>>>)` methods have been removed. These methods would have caused overloading issues in {VertX} {4}. The version of the method returning `Future<Message<T>>` would collide with the fire and forget version.
...
The following is an example of request and reply to a message in {VertX} {3x} releases:
...
The following is an example of request and reply to a message in {VertX} {4}:
