...
In {VertX} {4}, the clustering SPI has been redesigned. The subscription data model has changed. As a result of this, {VertX} {3x} nodes and {VertX} {4} nodes cannot be added together in the same Infinispan cluster.

The {VertX} applications are not impacted by this change as the EventBus and SharedData APIs remain the same.

If you had a custom Infinispan configuration file in your {VertX} {3x} application:
