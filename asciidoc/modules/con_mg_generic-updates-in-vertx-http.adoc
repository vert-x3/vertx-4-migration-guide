...
In {VertX} {v3x}, you could use the the HTTP client pool size to define the maximum number of websockets connections in an application. The value accessor methods `HttpClientOptions.maxPoolSize()` were used to get and set the websocket connections. The default number of connections was set to 4 for each endpoint.
