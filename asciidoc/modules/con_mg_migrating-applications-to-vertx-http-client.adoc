...
The HTTP client has been updated and improved in {VertX} {4}:
...
The following example shows how to send a GET request in {VertX} {3x} releases:
...
The following example shows how to send a GET request in {VertX} {4} release:
...
The following example shows you how to use future composition in GET operation in {VertX} {4}:
...
In {VertX} {3x} releases, you can use the `end()` method to send requests.
...
In {VertX} {4}, you can perform all the operations shown in the examples using the `get()` method. Additionally, you can also use the new `send()` method to perform these operations. You can pass a buffer, a string or a `ReadStream` as input to the `send()` method. The method returns an `HttpClientResponse` instance.
...
response.exceptionHandler(err -> {
  // Response failedxxx, something went wrong
});
----
+
