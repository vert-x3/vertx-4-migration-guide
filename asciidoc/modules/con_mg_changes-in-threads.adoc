...
The `Vertx.getOrCreateContext()` creates a single context for each non {VertX} thread. In releases prior to {VertX} {4}, a new context was created each time the method was called from a non {VertX} thread.
...
To get the same results in {VertX} {4}, you must update the code:
----
for (int i = 0;i < n;i++) {
