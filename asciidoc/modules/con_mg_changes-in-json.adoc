...
Users that must use both {VertX} {v3x} and {VertX} {v4} releases may need to create some utility to convert from Base64
to Base64URL. Given that both encodings are Base64, we're just changing the alphabet which makes this a trivial task:
...
Users should consider using these utility methods for:
