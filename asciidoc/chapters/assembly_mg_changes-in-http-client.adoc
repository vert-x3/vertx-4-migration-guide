...
{VertX} web client::
Use the {VertX} web client when your applications are HTTP oriented. For example, REST, encoding and decoding HTTP payloads, interpreting the HTTP status response code, and so on.

{VertX} HTTP client::
Use the {VertX} HTTP client when your applications are used as HTTP proxy. For example, as an API gateway. The HTTP client has been updated and improved in {VertX} {4}.
